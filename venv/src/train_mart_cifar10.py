...
# 调整学习率
def adjust_CIFAR10_learning_rate(epoch, optimizer):
    # 最小的学习率
...
def main(args):
    ...
    # 设置优化器
    optimizer = optim.SGD(resnet_model.parameters(), lr=CIFAR10_Training_Parameters['learning_rate'],
                          momentum=CIFAR10_Training_Parameters['momentum'],
                          weight_decay=CIFAR10_Training_Parameters['decay'], nesterov=True)
    # 训练
    # 最好的验证集精度
    best_val_acc = None
    # 训练模型参数保存路径：/CIFAR10/model/CIFAR10_raw.pt
    # **************不同模型需要修改名称**************
    model_saver = './CIFAR10/model/MART_ResNet18_' + 'raw' + '.pt'
    # 进行epoch次循环训练
    for epoch in range(CIFAR10_Training_Parameters['num_epochs']):
        ...
        # 学习率调整
        adjust_CIFAR10_learning_rate(optimizer=optimizer, epoch=epoch)
