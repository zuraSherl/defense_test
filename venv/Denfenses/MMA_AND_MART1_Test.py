...
from Denfenses.DefenseMethods.MMA_AND_MART1 import MMAANDMART1Defense
from Denfenses.DefenseMethods.MMA_AND_MART1 import add_indexes_to_loader
...
def main(args):
    ...
    # 将参数传入MMAANDMART1防御中
    mmaandmart1 = MMAANDMART1Defense(loader=train_loader, dataname="train", verbose=True, model=model_framework, defense_name=defense_name, dataset=dataset, training_parameters=training_parameters, device=device,
                     **mmaandmart1_params)
    # 进行对抗训练
    mmaandmart1.test_defense(validation_loader=valid_loader)

if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='The MMAANDMART1 Defenses')
    # dataset：数据集名称MNIST/CIFAR10
