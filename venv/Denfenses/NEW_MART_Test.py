...
from Denfenses.DefenseMethods.NEW_MART import NEW_MARTDefense
...
def main(args):
    ...
    # 将参数传入NEW_MART防御中
    new_mart = NEW_MARTDefense(model=model_framework, defense_name=defense_name, dataset=dataset, training_parameters=training_parameters, device=device,
                     **new_mart_params)
    # 利用MART进行防御
    new_mart.defense(train_loader=train_loader, validation_loader=valid_loader)

if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='The NEW_MART Defenses')
