...
from Denfenses.DefenseMethods.RLT import RLTDefense
...
def main(args):
    ...
    # 将参数传入RLT防御中
    rlt = RLTDefense(model=model_framework, defense_name=defense_name, dataset=dataset, training_parameters=training_parameters, device=device,
                     **rlt_params)
    # 进行对抗训练
    rlt.defense(train_loader=train_loader, validation_loader=valid_loader)


if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='The RLT Defenses')
    # dataset：数据集名称MNIST/CIFAR10
    # gpu_index：gpu数量，默认为0
    # seed：随机种子，默认为100
    # adv_ratio：对抗训练中对抗样本在损失函数中的权重，默认为0.3，论文里面使用的也是0.3
    # clip_min：随机扰动大小的下界，默认为0.0
    # clip_max：随机扰动大小的上界，默认为0.3
    # eps_mu：产生随机扰动大小正态分布的均值，默认为0
    # eps_sigma：产生随机扰动大小正态分布的方差，默认为50
    ...
    # **********根据数据集不同选择**********
    parser.add_argument('--epsilon', type=float, default=0.3, help='the random distribution of rllc attack')
