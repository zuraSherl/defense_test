...
class RLT3Defense(Defense):
    def __init__(self, model=None, defense_name=None, dataset=None, training_parameters=None, device=None, **kwargs):
        ...
        super(RLT3Defense, self).__init__(model=model, defense_name=defense_name)
        self.model = model
    ...
    def random_llc_generation(self, var_natural_images=None):
        ...
        # clone_var_natural_images = clone_var_natural_images + torch.zeros_like(clone_var_natural_images).uniform_(-self.eps, self.eps)
        # clone_var_natural_images = np.clip(clone_var_natural_images, 0.0, 1.0).astype(np.float32)
        # 损失函数对输入求梯度，并使用符号函数
        ...
        # 产生对抗样本
        ret_adv_images = clone_var_natural_images + self.alpha * gradients_sign
