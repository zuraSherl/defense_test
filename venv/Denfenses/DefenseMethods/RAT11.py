...
class RAT11Defense(Defense):
    def __init__(self, model=None, defense_name=None, dataset=None, training_parameters=None, device=None, **kwargs):
        ...
        super(RAT11Defense, self).__init__(model=model, defense_name=defense_name)
        self.model = model
    ...
    def _parsing_parameters(self, **kwargs):
        ...
        self.rllc_eps = kwargs['rllc_eps']
        self.rllc_eps_mu = kwargs['rllc_eps_mu']
        self.rllc_eps_sigma = kwargs['rllc_eps_sigma']
        self.rllc_clip_eps_min = kwargs['rllc_clip_eps_min']
        self.rllc_clip_eps_max = kwargs['rllc_clip_eps_max']
        return True

    # 用RLLC方法产生对抗样本（每个llc对抗样本的扰动是随机的），返回对抗样本
    def random_llc_generation(self, var_natural_images=None):
        ...
        random_eps = np.random.normal(loc=self.rllc_eps_mu, scale=self.rllc_eps_sigma,
                                      size=[var_natural_images.size(0)]) / 255.0
        # 将产生的随机数大小控制在[clip_eps_min，clip_eps_max]范围内
        random_eps = np.clip(np.abs(random_eps), self.rllc_clip_eps_min, self.rllc_clip_eps_max)
        # ****************添加的随机扰动*****************
        # 添加一个随机扰动，将扰动范围限制在[0,1]之间
        with torch.no_grad():
            random_distribution = torch.zeros_like(var_natural_images).uniform_(-self.rllc_eps, self.rllc_eps).to(self.device)
    ...
    def train_one_epoch_with_adv(self, train_loader, epoch):
        ...
        for index, (images, labels) in enumerate(train_loader):
            ...
            self.model.eval()
            # 为训练集产生对抗样本
            adv_rfgsm_images = self.random_rfgsm_generation(var_natural_images=nat_images)
            adv_rllc_images = self.random_llc_generation(var_natural_images=nat_images)
            self.model.train()
            # rfgsm对抗样本的损失函数
            logits_rfgsm_adv = self.model(adv_rfgsm_images)
            loss_fgsm_adv = F.cross_entropy(logits_rfgsm_adv, nat_labels)
            # rllc对抗样本的损失函数
            logits_rllc_adv = self.model(adv_rllc_images)
            loss_rllc_adv = F.cross_entropy(logits_rllc_adv, nat_labels)
            # 最终的损失函数：将两部分的loss按照权重相加再除以权重之和
            loss = (loss_rfgsm_adv + self.adv_ratio * loss_rllc_adv) / (1.0 + self.adv_ratio)
