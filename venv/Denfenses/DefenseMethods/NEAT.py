...
class NEATDefense(Defense):
    def __init__(self, model=None, defense_name=None, dataset=None, training_parameters=None, device=None, **kwargs):
        ...
        # kwargs：防御参数
        super(NEATDefense, self).__init__(model=model, defense_name=defense_name)
        self.model = model
    ...
    # 加载预训练模型，返回加载的模型
    def load_external_model_group(self, model_dir='../DefenseEnhancedModels/NEAT/', test_loader=None):
        ...
        for i in range(len(model_group)):
            ...
            # model_path：模型所在位置为DefenseEnhancedModels/NEAT/MNIST_NEAT_0.pt或者为CIFAR10
            model_path = '{}{}_NEAT_{}.pt'.format(model_dir, self.Dataset, str(i))
    ...
    def random_fgsm_generation(self, model=None, natural_images=None):
        ...
        # 首先对干净样本添加随机扰动
        with torch.no_grad():
            random_distribution = torch.zeros_like(natural_images).uniform_(-self.epsilon, self.epsilon).to(self.device)
        ...
        with torch.no_grad():
            ...
            xs_adv = new_images + self.alpha * torch.sign(gradient)
