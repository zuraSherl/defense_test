...
# MMA防御
class DEEPFOOLMMAMARTDefense(Defense):
    def __init__(self, loader, dataname="train", verbose=True, model=None, defense_name=None, dataset=None, training_parameters=None, device=None, **kwargs):
        ...
        super(DEEPFOOLMMAMARTDefense, self).__init__(model=model, defense_name=defense_name)
        self.model = model
    ...
    def train_one_batch(self, data, idx, target):
        ...
        # 第二个指标函数KL
        softm_nat = F.softmax(target, dim=1)
        ...
        loss = loss_adv_bce + self.lamda * loss_KL_pyi
