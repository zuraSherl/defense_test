...
# MMA防御
class DEEPFOOLMMADefense(Defense):
    def __init__(self, loader, dataname="train", verbose=True, model=None, defense_name=None, dataset=None, training_parameters=None, device=None, **kwargs):
        ...
        super(DEEPFOOLMMADefense, self).__init__(model=model, defense_name=defense_name)
        self.model = model
    ...
    def train_one_batch(self, data, idx, target):
        ...
        # 如果正确分类的图片数目大于0
        if num_correct > 0:
            # 为正确分类的样本产生DeepFool对抗样本，返回DeepFool对抗样本和DeepFool扰动
            advdata_correct, curr_eps_correct = self.deepfool_generation(data_correct,self.device)
            # 用DeepFool对抗样本替换正确分类的样本，更新data
            data[cln_correct] = advdata_correct
            # 更新正确分类样本添加的扰动，错误分类的保持为0
            curr_eps[cln_correct] = curr_eps_correct
        # 模型预测输出（正确分类的添加DeepFool扰动后的分类结果，错误分类则为原样本的分类结果）
        ...
        # 返回经过模型一个batch的logit输出、训练集的平均交叉熵损失和当前扰动（0/DeepFool扰动）
