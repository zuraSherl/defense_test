...
class NEW_MARTDefense(Defense):
    def __init__(self, model=None, defense_name=None, dataset=None, training_parameters=None, device=None, **kwargs):
        ...
        # kwargs：防御参数
        super(NEW_MARTDefense, self).__init__(model=model, defense_name=defense_name)
        self.model = model
    ...
    def train_one_epoch_with_pgd_and_nat(self, train_loader, epoch):
        ...
        for index, (images, labels) in enumerate(train_loader):
            ...
            loss = loss_adv_bce + self.lamda * (loss_KL_pyi + loss3 * loss_nat)
            # 干净样本和对抗样本的总loss
