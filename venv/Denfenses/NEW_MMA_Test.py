...
from Denfenses.DefenseMethods.NEW_MMA import NEW_MMADefense
...
def main(args):
    ...
    # 将参数传入NEW_MMA防御中
    new_mma = NEW_MMADefense(model=model_framework, defense_name=defense_name, dataset=dataset, training_parameters=training_parameters, device=device,
                     **new_mma_params)
    # 利用MART进行防御
    new_mma.defense(train_loader=train_loader, validation_loader=valid_loader)

if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='The NEW_MMA Defenses')
