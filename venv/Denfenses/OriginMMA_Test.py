...
from Denfenses.DefenseMethods.OriginMMA import OriginMMADefense
from Denfenses.DefenseMethods.OriginMMA import add_indexes_to_loader
...
def main(args):
    ...
    # 将参数传入OriginMMA防御中
    originmma = OriginMMADefense(loader=train_loader, dataname="train", verbose=True, model=model_framework, defense_name=defense_name, dataset=dataset, training_parameters=training_parameters, device=device,
                     **originmma_params)
    # 进行对抗训练
    originmma.test_defense(validation_loader=valid_loader)

if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='The OriginMMA Defenses')
    # dataset：数据集名称MNIST/CIFAR10
