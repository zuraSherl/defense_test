...
from Denfenses.DefenseMethods.NEW_MART1 import NEW_MART1Defense
...
def main(args):
    ...
    # 将参数传入NEW_MART1防御中
    new_mart1 = NEW_MART1Defense(model=model_framework, defense_name=defense_name, dataset=dataset, training_parameters=training_parameters, device=device,
                     **new_mart1_params)
    # 利用MART进行防御
    new_mart1.defense(train_loader=train_loader, validation_loader=valid_loader)

if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='The NEW_MART1 Defenses')
