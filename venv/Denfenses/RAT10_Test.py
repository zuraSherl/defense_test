...
from Denfenses.DefenseMethods.RAT10 import RAT10Defense
...
def main(args):
    ...
    rat10_params = {
        ...
        'llc_eps_mu': args.llc_eps_mu,
        'llc_eps_sigma': args.llc_eps_sigma,
        'llc_clip_eps_min': args.llc_clip_eps_min,
        'llc_clip_eps_max': args.llc_clip_eps_max
    }
    # 将参数传入RAT10防御中
    rat10 = RAT10Defense(model=model_framework, defense_name=defense_name, dataset=dataset, training_parameters=training_parameters, device=device,
                     **rat10_params)
    # 进行对抗训练
    rat10.defense(train_loader=train_loader, validation_loader=valid_loader)


if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='The RAT10 Defenses')
    # dataset：数据集名称MNIST/CIFAR10
    ...
    # **********根据数据集不同选择**********
    parser.add_argument('--llc_eps_mu', type=int, default=0, help='the \mu value of normal distribution for epsilon')
    # **********根据数据集不同选择**********
    parser.add_argument('--llc_eps_sigma', type=int, default=50, help='the \sigma value of normal distribution for epsilon')
    # **********根据数据集不同选择**********
    parser.add_argument('--llc_clip_eps_min', type=float, default=0.0, help='the min of epsilon allowed')
    # **********根据数据集不同选择**********
    parser.add_argument('--llc_clip_eps_max', type=float, default=0.3, help='the max of epsilon allowed')
