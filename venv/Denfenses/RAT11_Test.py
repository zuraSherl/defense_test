...
from Denfenses.DefenseMethods.RAT11 import RAT11Defense
...
def main(args):
    ...
    rat11_params = {
        ...
        'rllc_eps': args.rllc_eps,
        'rllc_eps_mu': args.rllc_eps_mu,
        'rllc_eps_sigma': args.rllc_eps_sigma,
        'rllc_clip_eps_min': args.rllc_clip_eps_min,
        'rllc_clip_eps_max': args.rllc_clip_eps_max
    }
    # 将参数传入RAT11防御中
    rat11 = RAT11Defense(model=model_framework, defense_name=defense_name, dataset=dataset, training_parameters=training_parameters, device=device,
                     **rat11_params)
    # 进行对抗训练
    rat11.defense(train_loader=train_loader, validation_loader=valid_loader)

if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='The RAT11 Defenses')
    # dataset：数据集名称MNIST/CIFAR10
    ...
    # **********根据数据集不同选择**********
    parser.add_argument('--clip_eps_min', type=float, default=0.0, help='the min of epsilon allowed')
    # **********根据数据集不同选择**********
    parser.add_argument('--clip_eps_max', type=float, default=0.3, help='the max of epsilon allowed')
    # **********根据数据集不同选择**********
    parser.add_argument('--rllc_eps', type=float, default=0.3, help='the random distribution of rllc attack')
    # **********根据数据集不同选择**********
    parser.add_argument('--rllc_eps_mu', type=int, default=0, help='the \mu value of normal distribution for epsilon')
    # **********根据数据集不同选择**********
    parser.add_argument('--rllc_eps_sigma', type=int, default=50, help='the \sigma value of normal distribution for epsilon')
    # **********根据数据集不同选择**********
    parser.add_argument('--rllc_clip_eps_min', type=float, default=0.0, help='the min of epsilon allowed')
    # **********根据数据集不同选择**********
    parser.add_argument('--rllc_clip_eps_max', type=float, default=0.3, help='the max of epsilon allowed')
