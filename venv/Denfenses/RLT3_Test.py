...
from Denfenses.DefenseMethods.RLT3 import RLT3Defense
...
def main(args):
    ...
    # 将参数传入RLT3防御中
    rlt3 = RLT3Defense(model=model_framework, defense_name=defense_name, dataset=dataset, training_parameters=training_parameters, device=device,
                     **rlt3_params)
    # 进行对抗训练
    rlt3.defense(train_loader=train_loader, validation_loader=valid_loader)

if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='The RLT3 Defenses')
    # dataset：数据集名称MNIST/CIFAR10
    # gpu_index：gpu数量，默认为0
    # seed：随机种子，默认为100
    # adv_ratio：对抗训练中对抗样本在损失函数中的权重，默认为0.3，论文里面使用的也是0.3
    # eps：扰动范围，默认为0.3
    # alpha：LLC扰动大小，默认为0.01
    ...
    # **********根据数据集不同选择**********
    parser.add_argument('--eps', type=float, default=0.3, help='the \mu value of normal distribution for epsilon')
    # **********根据数据集不同选择**********
    parser.add_argument('--alpha', type=float, default=0.375, help='the \sigma value of normal distribution for epsilon')
