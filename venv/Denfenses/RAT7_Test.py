...
from Denfenses.DefenseMethods.RAT7 import RAT7Defense
...
def main(args):
    ...
    rat7_params = {
        ...
        'rllc_alpha': args.rllc_alpha
    }
    # 将参数传入RAT7防御中
    rat7 = RAT7Defense(model=model_framework, defense_name=defense_name, dataset=dataset, training_parameters=training_parameters, device=device,
                     **rat7_params)
    # 进行对抗训练
    rat7.defense(train_loader=train_loader, validation_loader=valid_loader)


if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='The RAT7 Defenses')
    # dataset：数据集名称MNIST/CIFAR10
    ...
    # **********根据数据集不同选择**********
    parser.add_argument('--rllc_eps', type=float, default=0.3,help='the weight of adversarial example when adversarial training')
    # **********根据数据集不同选择**********
    parser.add_argument('--rllc_alpha', type=float, default=0.375, help='the min of epsilon allowed')
