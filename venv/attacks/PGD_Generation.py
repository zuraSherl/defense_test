...
from attacks.attack_methods.PGD import PGDAttack
...
class PGDGeneration(Generation):
    def __init__(self, dataset, attack_name, targeted, raw_model_location, clean_data_location, adv_examples_dir, device, attack_batch_size, eps,
                 eps_iter, num_steps):
        super(PGDGeneration, self).__init__(dataset, attack_name, targeted, raw_model_location, clean_data_location, adv_examples_dir, device)
...
if __name__ == '__main__':
    ...
    # epsilon：最大扰动，默认值为0.3
    # epsilon_iter：迭代步长，默认值为0.05
    # num_steps：迭代次数，默认值为15
    # attack_batch_size：分组产生对抗样本的分组大小，默认值为100
    # ************数据集可选MNIST/CIFAR10************
